...
### Extract all sampled dos (dos of n_oo, n_oe for a given (p,q,k_o)) #
###   into one big array
#########################################
...
for dname in dnames:
    ...
    # NOTE: k_o and k_c are switched!!
    for this_kc in range(this_N+1):

        this_ko = this_N - this_kc
        idx_ko = np.digitize(this_ko, vals_ko) - 1 

        ds = np.load('{}/dos_p_{:02d}_q_{:02d}_kc_{:03d}.npz'.format(dname, this_p, this_q, this_kc))
