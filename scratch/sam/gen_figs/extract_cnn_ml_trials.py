...
all_perf_tot[:] = np.nan
all_perf_test = np.zeros_like(all_perf_cv)
all_perf_test[:] = np.nan

all_n_params = np.zeros_like(all_perf_tot)
all_state_dict = np.zeros(xx.shape, dtype=object)
all_hyp_param_array = np.zeros_like(all_state_dict)
...
for i, fname in enumerate(fnames):
    ...
    all_perf_cv[x_idx, y_idx, z_idx] = mses_cv.mean()
    se_all_perf_cv[x_idx, y_idx, z_idx] = mses_cv.std(ddof=1)
    ...
    print("n_conv_filters: {} n_hidden_layer: {} n_node_hidden: {} mse: {:.2f} mse (CV'd): {:.2f} ({:.2f}) n_params: {}".format(n_conv_filters, n_hidden_layer, n_node_hidden, mse, mses_cv.mean(), mses_cv.std(ddof=1), n_params))
