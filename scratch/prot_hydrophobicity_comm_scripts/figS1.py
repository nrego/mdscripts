...
for idx, name in enumerate(names):
    ...
    ax1.bar(indices[idx], n_phob_res, width=width, label=name, color=colors[idx])
    ax2.bar(indices[idx], n_phob_surf, width=width, color=colors[idx])
    ...
    ax3.bar(indices[idx], n_res_surf, width=width, label=label, color=colors[idx])
