...
for i_round, (train_loader, test_loader) in enumerate(data_partition_gen):
    ...
    net = SAMGraphNet3L(adj_mat, n_hidden=12, n_out=outdim)
...
for i_round, (train_loader, test_loader) in enumerate(data_partition_gen):
    ...
    print("...Training")
    losses = train(net, criterion, train_loader, test_loader, do_cnn, learning_rate=0.0001, weight_decay=0.0, epochs=6000, break_out=3, loss_fn=loss_poly, loss_fn_args=(p_min, p_range_mat))
    print("    DONE...")
    print("\n")
    print("...Testing round {}".format(i_round))
    test_X, test_y = iter(test_loader).next()
